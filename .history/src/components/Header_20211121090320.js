...
export class Header extends Component {
    render(state) {
        let ListCart = [];
        let TotalCart=0;
        Object.keys(state.items.Carts).forEach(function(item){
            TotalCart+=state.items.Carts[item].quantity * state.items.Carts[item].price;
            ListCart.push(state.items.Carts[item]);
        });
